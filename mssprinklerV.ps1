function Invoke-MSSprinklerV{
    ...
    # Initialize a hashtable to keep track of usernames to skip
    $skipUsernames = @{}
    
    foreach ($passes in $passwords) {
        foreach ($un in $usernames) {
            if ($skipUsernames.ContainsKey($un)) {
                continue
            }
            # Web Request Info
            ...
            if ($response.StatusCode -eq 200) {
                ...
                # No need to attempt further passwords for this user
                $skipUsernames[$un] = $true
            } else {
                ...
                if ($errorResponseString -match 'AADSTS\d{5}') {
                    ...
                    if ($errorResonseValues.ContainsKey($errorCode)) {
                        ...
                        if ($errorCode -eq "AADSTS50059" -or $errorCode -eq "AADSTS50034" -or $errorCode -eq "AADSTS50053") {
                            ...
                            # Skip further attempts for this user
                            $skipUsernames[$un] = $true
                        } else {
                            Write-Host "$message"
